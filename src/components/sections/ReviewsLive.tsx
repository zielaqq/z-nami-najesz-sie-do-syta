 ...
 *  • Jeśli odwiedzający zgodził się w banerze cookies (patrz `CookieConsent`) – ładują się same, gdy sekcja
 *    zbliża się do ekranu (tak jak mapa i nagrania z Facebooka).
 *  • Bez zgody (albo przed decyzją) – dopiero po kliknięciu „Pokaż opinie z Google”; samo wejście na stronę
 *    (i samo przewinięcie do sekcji) nie wysyła wtedy żadnego zapytania. Kliknięcie działa zawsze, niezależnie
 *    od banera – to samo w sobie świadoma zgoda.
 * Regulamin Google zabrania cache'owania treści Places, więc opinie nigdy nie są zapisywane ani wbudowane w stronę;
 * dzienny limit zapytań i alert budżetu ustawia się w Google Cloud (patrz docs/GOOGLE-OPINIE.md).
 */
export function ReviewsLive({ googleUrl }: ReviewsLiveProps) {
  ...
  // Automatyczne wczytanie: tylko po zgodzie w banerze cookies i gdy sekcja zbliża się do ekranu (z zapasem 500 px).
  useEffect(() => {
    if (consent !== "accepted" || state.status !== "waiting") return;
      ...
      {state.status === "ready" ? (
        ...
        <div className="max-w-xl border-t-2 border-ink bg-white p-6 sm:p-8">
          ...
          <p className="mt-2 text-mute">
            {consent === "accepted"
              ? "Zaraz się wczytają…"
              : "Pobieramy je na żywo z Google dopiero po kliknięciu (albo od razu po zgodzie w banerze cookies)."}
          </p>
